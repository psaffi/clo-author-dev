...
*Family Level Lending (all and passive)
****************************************
...
bys ui_family ui_dscd datem: egen temp2 = sum(lsec_mv_lend_adj) if dum_fund_active == 0
gen lon_loan_fam_pas = temp/(lMV_eur*10^3)
replace lon_loan_fam_pas = lon_loan_fam_pas*100

gcollapse (max) lon_loan_fam_pas, by(ui_family ui_dscd datem) merge replace

winsor2 lon_loan_fam, cut(0 99) replace
winsor2 lon_loan_fam_pas, cut(0 99) replace
...
gen lon_loan_fam_pas_d = lon_loan_fam_pas

qui sum lon_loan_fam_pas 
replace lon_loan_fam_pas_d = (lon_loan_fam_pas_d - r(mean))/r(sd) 

label variable lon_loan_fam_pas_d "Amount on Loan of a given stock by a all passive funds of fam. f"
...
foreach s of global SD{
	foreach d of global dependent{
		...
		****************************************
		reghdfe `d'  c.lon_loan_fam_pas_d  ${fund_sec_controls} if dum_exit == 0 & dum_fund_active == 1 & disclosure_sample == 1 & ever_LSP == 1 & first_LSP_6m_pre1 != . & lon_loan_fund != ., absorb(ui_dscd ui_fund datem) cluster(${clustvar})
		est store reg_dpos_pool_1
		
		****************************************
		*2.Regression: Include interaction with OnLoan of Stock
		****************************************
		reghdfe `d'  c.lon_loan_fam_pas_d c.first_LSP_6m_pre1  ${fund_sec_controls} if dum_exit == 0 & dum_fund_active == 1 & disclosure_sample == 1 & ever_LSP == 1 & first_LSP_6m_pre1 != . & lon_loan_fund != ., absorb(ui_dscd ui_fund datem) cluster(${clustvar})
		...
		****************************************
		reghdfe `d'  c.lon_loan_fam_pas_d##c.first_LSP_6m_pre1  ${fund_sec_controls} if dum_exit == 0 & dum_fund_active == 1 & disclosure_sample == 1 & ever_LSP == 1 & first_LSP_6m_pre1 != . & lon_loan_fund != ., absorb(ui_sec_datem ui_fund_datem ui_fund_sec)  cluster(${clustvar})
		est store reg_dpos_pool_4
		
		****************************************
		*5.Regression: Include interaction with OnLoan of Stock
		****************************************
		qui reghdfe `d' lon_loan_fund_d ${fund_sec_controls} if dum_exit == 0 & dum_fund_active == 1, absorb(ui_sec_datem ui_fund_datem ui_fund_sec) cluster(${clustvar})
		capture drop include
		gen include = e(sample)
		
		ivreghdfe `d'  (lon_loan_fund_d = lon_loan_fam_pas_d)  ${fund_sec_controls} if include == 1, absorb(ui_dscd ui_fund datem) cluster(${clustvar}) first savefirst
		...
		****************************************
		esttab reg_dpos_pool* _ivreg2_lon_loan_fund_d using "temp.csv", b(4) t(2) star(* 0.10 ** 0.05 *** 0.01) /// 
		...
		****************************************
		...
		export excel using "${datum}_2SLS_SecLend_NEW_T10.xlsx", sheet("NEW Table 10 - F,T") sheetreplace   cell(B5)
