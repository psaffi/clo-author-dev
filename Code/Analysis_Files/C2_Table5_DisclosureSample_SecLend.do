...
foreach s of global SD{
	foreach d of global dependent{
		...
		****************************************
		reghdfe `d'  c.lon_loan_fund_d ${fund_sec_controls} if dum_exit == 0 & dum_fund_active == 1 & disclosure_sample == 1 & ever_LSP == 1 & first_LSP_6m_pre1 != . & lon_loan_fund != ., absorb(ui_dscd ui_fund datem)  cluster(${clustvar})
		est store reg_dpos_pool_1
		
		****************************************
		*2.Regression: Include interaction with OnLoan of Stock
		****************************************
		reghdfe `d'  c.lon_loan_fund_d c.first_LSP_6m_pre1 ${fund_sec_controls} if dum_exit == 0 & dum_fund_active == 1 & disclosure_sample == 1 & ever_LSP == 1 & first_LSP_6m_pre1 != . & lon_loan_fund != ., absorb(ui_dscd ui_fund datem)  cluster(${clustvar})
		...
		****************************************
		reghdfe `d'  c.lon_loan_fund_d##c.first_LSP_6m_pre1 ${fund_sec_controls} if dum_exit == 0 & dum_fund_active == 1 & disclosure_sample == 1 & ever_LSP == 1 & first_LSP_6m_pre1 != . & lon_loan_fund != ., absorb(ui_fund_sec datem)  cluster(${clustvar})
		est store reg_dpos_pool_4
		
		****************************************
		*5.Regression: Include interaction with OnLoan of Stock
		****************************************
		reghdfe `d'  c.lon_loan_fund_d##c.first_LSP_6m_pre1 ${fund_sec_controls} if dum_exit == 0 & dum_fund_active == 1 & disclosure_sample == 1 & ever_LSP == 1 & first_LSP_6m_pre1 != . & lon_loan_fund != ., absorb(ui_sec_datem ui_fund)  cluster(${clustvar})
		est store reg_dpos_pool_5
		
		****************************************
		*5.Regression: Include interaction with OnLoan of Stock
		****************************************
		reghdfe `d'  c.lon_loan_fund_d##c.first_LSP_6m_pre1 ${fund_sec_controls} if dum_exit == 0 & dum_fund_active == 1 & disclosure_sample == 1 & ever_LSP == 1 & first_LSP_6m_pre1 != . & lon_loan_fund != ., absorb(ui_fund_datem ui_dscd)  cluster(${clustvar})
		est store reg_dpos_pool_6
		
		****************************************
		*4.Regression: Include interaction with OnLoan of Stock
		****************************************
		reghdfe `d'  c.lon_loan_fund_d##c.first_LSP_6m_pre1 ${fund_sec_controls} if dum_exit == 0 & dum_fund_active == 1 & disclosure_sample == 1 & ever_LSP == 1 & first_LSP_6m_pre1 != . & lon_loan_fund != ., absorb(ui_sec_datem ui_fund_datem ui_fund_sec)  cluster(${clustvar})
		est store reg_dpos_pool_7
		
		****************************************
		*5.Regression: Include interaction with OnLoan of Stock
		****************************************
		reghdfe `d'  c.lon_loan_fund_d##c.first_LSP_6m_pre1 ${fund_sec_controls} if dum_exit == 0 & dum_fund_active == 0 & disclosure_sample == 1 & ever_LSP == 1 & first_LSP_6m_pre1 != . & lon_loan_fund != ., absorb(ui_sec_datem ui_fund_datem ui_fund_sec)  cluster(${clustvar})
		...
		****************************************
		...
		export excel using "${datum}_Holdings_SecLend_NEW_T5.xlsx", sheet("NEW Table 5 - F,T") sheetreplace   cell(B5)
