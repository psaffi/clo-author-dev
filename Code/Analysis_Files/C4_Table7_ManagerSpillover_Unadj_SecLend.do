...
****************************************
	foreach s of global SD{
		...
		foreach d of global dependent{
			...
			****************************************
			reghdfe `d' c.lon_loan_fund_d##c.first_LSP_6m_pre1 c.lon_loan_man_other_fund_d##c.first_LSP_6m_pre1 ${fund_sec_controls}   if dum_exit == 0 & disclosure_sample == 1 & ever_LSP == 1 & first_LSP_6m_pre1 != . & lon_loan_fund != ., absorb(ui_fund_datem ui_sec_datem ui_fund_sec) cluster(${clustvar})
			est store reg_dpos_pool_2
			
			****************************************
			*3.Regression: Manager has lent (funds not managed by him w/o active lending)
			****************************************
			reghdfe `d' c.lon_loan_man_d##c.first_LSP_6m_pre1 c.lon_loan_fam_other_man_d##c.first_LSP_6m_pre1 ${fund_sec_controls}   if dum_exit == 0 & disclosure_sample == 1 & ever_LSP == 1 & first_LSP_6m_pre1 != . & lon_loan_fund != ., absorb(ui_fund_datem ui_sec_datem ui_fund_sec) cluster(${clustvar})
			est store reg_dpos_pool_3
			
			****************************************
			*4.Regression: All Combined
			****************************************
			reghdfe `d' c.lon_loan_fund_d##c.first_LSP_6m_pre1 c.lon_loan_man_other_fund_d##c.first_LSP_6m_pre1 c.lon_loan_fam_other_man_d##c.first_LSP_6m_pre1 ${fund_sec_controls}   if dum_exit == 0 & disclosure_sample == 1 & ever_LSP == 1 & first_LSP_6m_pre1 != . & lon_loan_fund != ., absorb(ui_fund_datem ui_sec_datem ui_fund_sec) cluster(${clustvar})
			...
			*Export Excel File 
			****************************************
			export excel using "${datum}_Disclosure_Manager_NEW_T7.xlsx", sheet("Modi T7 - FF,T") sheetmodify   cell(B5)	
