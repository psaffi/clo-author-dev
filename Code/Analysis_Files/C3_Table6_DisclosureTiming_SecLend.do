...
foreach s of global SD{
	foreach d of global dependent{
		...
		****************************************
		reghdfe `d'  c.lon_loan_fund_d c.first_LSP_6m_pre1 c.first_LSP_6m c.first_LSP_6m_post1 ${fund_sec_controls} if dum_exit == 0 & dum_fund_active == 1 & disclosure_sample == 1 & ever_LSP == 1 & first_LSP_6m_pre1 != . & lon_loan_fund != ., absorb(ui_dscd ui_fund datem)  cluster(${clustvar})
		est store reg_dpos_pool_2
		
		****************************************
		*3.Regression: Include interaction with OnLoan of Stock
		****************************************
		reghdfe `d'  c.lon_loan_fund_d##c.first_LSP_6m_pre1 c.lon_loan_fund_d##c.first_LSP_6m c.lon_loan_fund_d##c.first_LSP_6m_post1 ${fund_sec_controls} if dum_exit == 0 & dum_fund_active == 1 & disclosure_sample == 1 & ever_LSP == 1 & first_LSP_6m_pre1 != . & lon_loan_fund != ., absorb(ui_dscd ui_fund datem) cluster(${clustvar})
		...
		****************************************
		reghdfe `d'  c.lon_loan_fund_d##c.first_LSP_6m_pre1 c.lon_loan_fund_d##c.first_LSP_6m c.lon_loan_fund_d##c.first_LSP_6m_post1 ${fund_sec_controls} if dum_exit == 0 & dum_fund_active == 1 & disclosure_sample == 1 & ever_LSP == 1 & first_LSP_6m_pre1 != . & lon_loan_fund != ., absorb(ui_sec_datem ui_fund_datem ui_fund_sec)  cluster(${clustvar})
		...
		****************************************
		...
		export excel using "${datum}_Holdings_SecLend_NEW_T6.xlsx", sheet("NEW Table 6 - FF,T") sheetreplace   cell(B5)
