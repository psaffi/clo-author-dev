...
\noindent where $Y_{i,j,t}$ is the monthly percentage change in the number of shares of stock $i$ held by fund $j$ at time $t$ ($PosChange_{i,j,t}$). $OnLoan_{i,j,t-1}$ is the dollar value of shares lent by fund $j$ in stock $i$ at $t-1$ divided by the market capitalization of stock $i$, measuring the fund's exposure to private short selling demand. $\mathbf{X}_{i,j,t-1}$ includes lagged position weight ($PosWeight_{i,j,t-1}$) and an Amihud~\citeyearpar{amihud:02} illiquidity rank ($ILLIQ_{i,j,t-1}$). Standard errors are clustered at the fund level. Variable definitions are provided in Appendix~\ref{app:variables}.
